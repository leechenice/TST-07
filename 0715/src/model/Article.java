    ...
    */
    ...
    public static void publish(int userId, String title, String content) throws SQLException {
        ...
        try (Connection c = DBUtil.getConnection()) {
            String sql = "INSERT INTO articles (user_id, title, context, published_at) VALUES (?, ?, ?, ?)";
    ...
    public static Article get(int id) throws SQLException {
        try (Connection c = DBUtil.getConnection()) {
            String sql = "select title, context from articles where id = ?";
            try (PreparedStatement s = c.prepareStatement(sql)) {
                s.setInt(1, id);
                try (ResultSet r = s.executeQuery()) {
                    if (!r.next()) {
                    ...
                    Article article = new Article();
                    article.title = r.getString("title");
                    article.content = r.getString("context");
